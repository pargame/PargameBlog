...
const IGNORES = new Set(['node_modules', '.git', 'dist', 'build', 'public', 'out', '.vite'])
...
function readTsconfigPaths() {
  try {
    ...
    for (const [key, arr] of Object.entries(paths)) {
      // key like '@lib/*' -> prefix '@lib/' and wildcard
...
function textSearchForFileUsage(filePath, allFiles) {
  // search for basename or relative path occurrences across files (cheap heuristic)
...
for (const f of allFiles) {
  ...
  const found = textSearchForFileUsage(f, allFiles)
  if (found) used.add(f)
}
...
// also provide JSON output file for downstream review
try {
